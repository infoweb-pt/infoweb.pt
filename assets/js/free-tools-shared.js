 ...
 *
 * Prefer `window.FT.*` for new code. Legacy globals (show, hide, …) are set
 * only if not already defined so per-tool scripts can override.
 */
(function () {
  'use strict';
  ...
  exposeGlobal('show', show);
  exposeGlobal('hide', hide);
  exposeGlobal('showFlex', showFlex);
  exposeGlobal('hideFlex', hideFlex);
  exposeGlobal('toggle', toggle);
  exposeGlobal('validateEmail', validateEmail);
  exposeGlobal('validatePhone', validatePhone);
  exposeGlobal('validateRequired', validateRequired);
  exposeGlobal('submitContactLead', submitContactLead);
  exposeGlobal('copyToClipboard', copyToClipboard);
  exposeGlobal('uploadFile', uploadFile);
  exposeGlobal('createSmartQR', createSmartQR);
  exposeGlobal('debounce', debounce);
  exposeGlobal('formatCurrency', formatCurrency);
  exposeGlobal('formatNumber', formatNumber);
  exposeGlobal('scrollToElement', scrollToElement);
